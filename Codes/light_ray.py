    ...
    def make_light_ray(self, seed=None, periodic=True,
        ...
        **Examples**
        ...
        if right_edge is None:
            ...
            right_edge = domain.arr(right_edge, assumed_units).copy()
        ...
        if start_position is not None:
            if hasattr(start_position, 'units'):
                ...
                start_position = self.ds.arr(start_position, assumed_units).copy()
        ...
        if end_position is not None:
            if hasattr(end_position, 'units'):
                ...
                end_position = self.ds.arr(end_position, assumed_units).copy()
        ...
        if ('gas', 'temperature_mu') not in fields:
           fields.append(('gas', 'temperature_mu'))
        data_fields = fields[:]
        all_fields = fields[:]
        ...
        data_fields = [('gas', 'density_2') if a==('gas', 'density') else a for a in data_fields]
        all_fields = [('gas', 'density_2') if a==('gas', 'density') else a for a in all_fields]
        data_fields = [('gas', 'temperature_mu') if a==('gas', 'temperature') else a for a in data_fields]
        all_fields = [('gas', 'temperature_mu') if a==('gas', 'temperature') else a for a in all_fields]
